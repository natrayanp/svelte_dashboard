...
onMount(()=> {
  authVal.allpack.companylevel.forEach( (e) => {
	...
	function drop(event, basket) {
        ...
        if(data.basketname !== basket.basketname) {
            if(data.basketname === "Availablemodules" && basket.basketname === "Selectedmodules"){                
                let intarget = roleVal.Selectedmodules.filter(x => x.name === data.name);                
                console.log(intarget);
                if(intarget.length === 0) {
                    //roleStore.Availablemodules = roleStore.Availablemodules.filter(x => !(x.name === data.name));
                    let newava = roleVal.Availablemodules.filter(x => !(x.name === data.name));                    
                    roleStore.setModVal({modname:"Availablemodules", value:newava});                    
                    roleStore.updateModVal({modname:"Selectedmodules", value:{...data,basketname:"Selectedmodules"}});
                }
            }
        }
    ...
    function removemodule(data) {              
        ...
        console.log(data);
        console.log(roleVal);
        console.log(roleVal.Availablemodules);
        //let intarget = roleStore.Availablemodules.filter(x => x.name === data.name);
        let intarget = roleVal.Availablemodules.filter(x => x.packid === data.packid);
        console.log(intarget);
        if(intarget.length === 0) {
          let newsel = roleVal.Selectedmodules.filter(x => !(x.packid === data.packid));
          roleStore.setModVal({modname:"Selectedmodules", value:newsel});
            //roleStore.Selectedmodules = roleStore.Selectedmodules.filter(x => !(x.name === data.name));
            //roleStore.Availablemodules.push({...data,basketname:"Availablemodules"});
          roleStore.updateModVal({modname:"Availablemodules", value:{...data,basketname:"Availablemodules"}});            
        }
        console.log(roleVal);
    }
...
<div class = "flex flex-col bg-white justify-center shadow rounded-lg p-3">
  ...
  <div class = "flex flex-col md:flex-row flex-wrap bg-blue-100 justify-center gap-x-5 gap-y-5 shadow rounded-lg p-3">
      ...
      <div class="shadow rounded-lg p-3  bg-gray-100 w-full md:w-5/12">            
          ...
          {/if}
          {#each $roleStore.Availablemodules as basket, basketIndex (basket)} 
          ...
          <div class="shadow rounded-lg p-3  bg-green-100 w-full md:w-9/19">
              <div class="flex flex-row justify-center">
                ...
                <div class="container">
                              <table class="min-w-full divide-y divide-gray-200 ">
                                     <!--         
                                {#each $roleStore.Selectedmodules[basketIndex].submenu as item, itemIndex (item)}           
                                ...
                                {#if item.submenu !== null}
                                  <thead  class="bg-blue-100">                           
                                      <tr>
                                          ...
                                          <th class="sticky top-0  py-3 text-red-900 "><input type="checkbox" on:change={()=>onReadChange(basketIndex)} class="h-4 w-4 text-red-600" 
                                              bind:checked={$roleStore.Selectedmodules[basketIndex].readAll} 
                                              bind:indeterminate={$roleStore.Selectedmodules[basketIndex].readindeterminate} /> Read</th>
                                          <th class="sticky top-0  py-3 text-blue-900 "><input type="checkbox" on:change={()=>onWriteChange(basketIndex)} class="h-4 w-4 text-blue-600" 
                                              bind:checked={$roleStore.Selectedmodules[basketIndex].writeAll}
                                              bind:indeterminate={$roleStore.Selectedmodules[basketIndex].writeindeterminate}/> Write</th>
                                  ...
                                  <tbody class="bg-white divide-y divide-gray-200 overflow-y-scroll h-28">                                        
                                      <tr>                                            
                                          ...
                                          <td class="px-10 py-1 whitespace-nowrap"><input type="checkbox" class="h-4 w-4 text-red-600" on:change={()=>onReadChangeIt(basketIndex,itemIndex)}  bind:checked={$roleStore.Selectedmodules[basketIndex].submenu[itemIndex].read}></td>
                                          <td class="px-6 py-1 whitespace-nowrap"><input type="checkbox" class="h-4 w-4 text-blue-600"  on:change={()=>onWriteChangeIt(basketIndex,itemIndex)} bind:checked={$roleStore.Selectedmodules[basketIndex].submenu[itemIndex].write}></td>
                                          <td></td>
                                      </tr>                                        
                                  </tbody>
                              {/if}
